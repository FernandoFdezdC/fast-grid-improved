...
const computeView = async ({ rows, buffer, viewConfig, shouldCancel, }) => {
    ...
    if (sortKey === cache.sortKey) {
        ...
        const sortResult = await timSort(rowsArr, getSortComparisonFn(sortConfig.map((c) => [c.direction, c.column])), shouldCancel);
        if (!sortResult.ok) {
            return "cancelled";
        }
