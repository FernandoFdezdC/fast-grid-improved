...
export const FastGrid = () => {
  ...
  useEffect(() => {
    ...
    const loadAndInitialize = async () => {
      try {
        // 1. Obtener archivo primero
        const csvFile = await getCSVFromPublic();
        
        // 2. Obtener columnas del CSV
        const csvColumns = await getCSVColumns(csvFile);
        
        // 3. Inicializar grid con las columnas reales
        const t0 = performance.now();
        const grid = new Grid(container, [], ['Index', ...csvColumns]);
        setGrid(grid);
        console.info("Ms to initialize grid:", performance.now() - t0);
  
        // 4. Cargar datos
        setLoadingRows(true);
        await loadCSV(csvFile, grid, () => setLoadingRows(false), rowCount);
